...
fig_notes <- c(
  "01_valis_rtre_by_stage" = paste(
    ...
    "The y axis is **clipped** at the per-stage Tukey fence so a single blown-up slide",
    "cannot flatten the rest onto the axis — nothing is dropped, and the caption states",
    "how many slides sit above the view."),
  ...
  "02_overlap_dice_by_stage" = paste(
    ...
    "stage (native → rigid → non_rigid → micro), one box across slides. Computed from",
    "DAPI-nucleus overlap, not VALIS features; higher = better. Boxes only: the per-slide",
    "trajectory is what §1 draws, and overlaying it here buried the medians."),
  "02b_displacement_um_by_stage" = paste(
    ...
    "**Log y axis** — the residual falls by two orders of magnitude across the ladder,",
    "so on a linear axis every registered stage collapses onto zero."),
