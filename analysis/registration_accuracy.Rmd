...
- **This is the SWEEP, not your samples.** These tables come from mirage's
  ...
  per-tile spatial error map — see [run QC](run_qc.html), which reads `data/mirage/`
  and needs no sweep.
- **This page owns sweep registration accuracy.** §4 and §5 read the same
  `param_matrix.csv` columns the [pipeline benchmarks](benchmarks.html) page used to
