...
Scatter of `ihc_frac` (x) vs `score` (y), **one figure per method**, panels by
lineage, in the ranking's order so the best-agreeing method comes first. Within a
figure the four panels share both axes: x is the IHC fraction on 0..1 and y is that
method's score, so the spread across lineages reads on one scale. Between figures
y is free by construction, because deconvolution scores are not on a common scale
from one method to the next. Each strip carries that lineage's Spearman rho from
the table above, so the picture and the number sit together.

```{r deconv-concordance-plot, eval = have_arm, fig.height=3.4}
if (!is.null(deconv_paired) && nrow(deconv_paired) > 0) {
