...
test_that("the deconvolution panel colours by the hotscore groups when given them", {
  ...
  expect_equal(levels(p$data$group)[1], "cold")
  # Facets follow the fixed population order, not alphabetical.
  expect_equal(levels(p$data$lineage), comparable_lineages)
})
